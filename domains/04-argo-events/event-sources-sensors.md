...
- EventSources capture events from external systems and publish to the event bus
- Sensors listen to events and trigger actions based on dependencies
- EventSource and Sensor are separate CRDs for separation of concerns
- Multiple event source types are supported (webhook, calendar, resource, message queues, etc.)
- Sensors can depend on multiple events and filter based on event data
- Event data can be extracted and passed to triggers using templates
- Sensors support various trigger types including Argo Workflows, K8s resources, and HTTP
- Event filtering can be done at the dependency level using data, context, or expression filters
- Calendar event sources provide scheduling similar to CronWorkflow but in event-driven context
- Resource event sources watch Kubernetes objects for state changes

## Hands-On Practice

- [Lab 02: Event Sources](../../labs/04-argo-events/lab-02-event-sources.md) - Create and configure EventSources and Sensors, test webhook events, implement resource watching
